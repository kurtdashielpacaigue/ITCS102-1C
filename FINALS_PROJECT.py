...
def Forloopinfo():
    ...
    print("\nHere is the exact code in the second option:")
    print("for uma in range(1, 11, 1):")
    print("    for deez in range(uma, 10, 1):")
    print("        print('*', end=' ')")
    print("    print()")

    print("\nThis creates a Reverse triangle pattern in the line.")
    input("\nPress any key to go back...")
...
def nestedforloopinfo():
    os.system('cls')
    print("\n--- INFORMATION ABOUT THE NESTED LOOP CODE ---")

    print("A nested for loop means a for loop inside another for loop.")
    print("The Outer loop runs first and controls the number of times the INNER loop runs.")
    print("For every single one round of the outer loop,\nthe inner loop runs completely from start to finish.")
...
while True:
    ...
    print("++++++++++++++++++++++++++++++\n")
    print("A - PRINT FUNCTION\t\tG - LIST FUNCTION")
    print("B - INPUT FUNCTION\t\tH - WHILE TRUE FUNCTION") 
    print("C - EVAL FUNCTION\t\tI - DICTIONARY FUNCTION")
    print("D - INT FUNCTION\t\tJ - DEF - FUNCTION")
    print("E - IF ELSE ELIF FUNCTION\tK - FORLOOP FUNCTION")
    print("F - LOOP FUNCTION\t\tL - NESTED FOR LOOP FUNCTION")
    ...
    if hutao == 'a':
        ...
        choice11(Forloopinfo,Forlooprun)
        continue
    elif hutao =='l':
        choice12(nestedforloopinfo,nestedforlooprun)
        continue
